...
namespace TRMIssues
{
    ...
    partial class IssuesForm : Form
    {
        ...
        private readonly PluginRedMine servicePluginRedmine;
        ...
        public IssuesForm(PluginRedMine plugin, string baseComment)
        {
            Thread.CurrentThread.CurrentUICulture = Thread.CurrentThread.CurrentCulture;
            servicePluginRedmine = plugin;
            InitializeComponent();
        ...
        {
            //Display Version
            lblVersion.Text = String.Format("{0} : {1}", Resources.FormsResources.VersionLabel, VersionCheck.CurrentVersion);
        ...
        private void myIssuesForm_Shown(object sender, EventArgs e)
        {
            ...
            lblStatus.Text = Resources.FormsResources.AtomLoading;
            var feedThread = new Thread(servicePluginRedmine.GetFeed) { CurrentUICulture = Thread.CurrentThread.CurrentCulture };
            feedThread.Start();
            ...
            if (servicePluginRedmine == null || servicePluginRedmine.FeedFailedToLoad)
            {
                lblStatus.Text = Resources.FormsResources.AtomLoadFailed;
                btnCancel.Enabled = true;
                return;                
            }

            lblStatus.Text = String.Format(Resources.FormsResources.AtomLoadComplete, servicePluginRedmine.FeedItems.Count);
            ...
            if (!String.IsNullOrEmpty(servicePluginRedmine.FeedTitle))
            {
                Text = servicePluginRedmine.FeedTitle;
            }

            //If new version available enable flashing color
            tmrNewVersion.Enabled = lblUpdateAvailable.Visible;
        }
        ...
        {
            //Define columns
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.Other.ToString(), "  ");
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.Numeric.ToString(), Resources.FormsResources.ColumnNumber);
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.Text.ToString(), Resources.FormsResources.ColumnType);
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.Text.ToString(), Resources.FormsResources.ColumnSummary);
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.Text.ToString(), Resources.FormsResources.ColumnStatus);
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.Text.ToString(), Resources.FormsResources.ColumnAuthor);
            lvwIssueList.Columns.Add(ListViewColumnSorter.ColumnType.DateTime.ToString(),Resources.FormsResources.ColumnLastUpdated);

            //Get Feed Items
            feedItems = servicePluginRedmine.FeedItems;
        ...
        private void loadListViewSettings()
        {
            ...
            lvwColumnSorter.Order = settings.SortOrder;
            ...
            foreach (var column in settings.ColumnsDisplay)
            {
        ...
        private void saveSettings()
        {
            ...
            foreach (ColumnHeader column in lvwIssueList.Columns)
            {
                ...
                {
                    settings.ColumnsDisplay.Add(column.Index, column.DisplayIndex);
                }
                else
                {
                    //Flag hidden column with negative display index
                    settings.ColumnsDisplay.Add(column.Index, -1*column.DisplayIndex);
                }
        ...
        private void restoreRecentItems()
        {
            ...
            foreach (var item in SettingsManager.GetRecentItems())
            {
        ...
        {
            SettingsManager.SaveRecentItems(listItems);
        }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                gbxSearch.Text = Resources.FormsResources.SearchBoxNoResults;
                restoreRecentItems();
            }
            else
            {
                var nbResu = findIssue(txtSearch.Text, colorSuccess, colorFailed, false);
                gbxSearch.Text = String.Format(Resources.FormsResources.SearchBoxWithResults, nbResu);
            }
        }
            ...
            foreach (ListViewItem item in lvwIssueList.Items)
            {
                if (textNoCase != string.Empty)
                {
                    for (var i = 0; i < item.SubItems.Count; i++)
                    {
                        ...
                        if (cboFields.SelectedItem != null && cboFields.SelectedItem.ToString() != AllFields && cboFields.SelectedItem.ToString() != lvwIssueList.Columns[i].Text)
                        {
                            continue;
                        }
        ...
        private void btnTimeEntry_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem item in lvwIssueList.CheckedItems)
            {
                System.Diagnostics.Process.Start(Uri.EscapeUriString(((FeedItem)item.Tag).TimeEntryUrl + comment));
            }
        }
