...
namespace TurtleMine
{
	...
	internal class FeedParser
	{
		...
		public FeedParser(String feedUrl)
		{
			...
			{
				throw new ArgumentException(Strings.FeedParser_feedUrl_is_not_provided, "feedUrl");
			}

			//Read the Url into a Syndication Feed.
			var reader = ConnectionHelper.CreateXmlReader(feedUrl, ConnectionHelper.GetDefaultProxy());
			...
			{
				baseRedmineUrl = baseRedmineUrl.Remove(baseRedmineUrl.LastIndexOf("welcome", StringComparison.CurrentCultureIgnoreCase));
			}
